...
import * as fn_utils from './functional';
// import logger from './utils/logger';
import { isWhitespace } from './text';
import Path from './path';
// import { InMemory } from './backend/shared';
import {
...
export default class Document extends EventEmitter {
    ...
    public async _newChild(parent: Row, index = -1): Promise<AttachedChildInfo> {
        const row = 0; //  await this.store.getNew();
    ...
    public async getInfo(row: Row): Promise<CachedRowInfo> {
        ...
        const [
            ...
            new Array<Char>(), // this.store.getLine(row),
            true, // this.store.getCollapsed(row),
            new Array<Row>(), //  this.store.getChildren(row),
            new Array<Row>(), //this.store.getParents(row),
    ...
    public async _attach(child_row: Row, parent_row: Row, index = -1): Promise<AttachedChildInfo> {
        ...
        const old_detached_parent = null; // await this.store.getDetachedParent(child_row);
