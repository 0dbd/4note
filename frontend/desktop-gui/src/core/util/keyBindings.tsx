...
export class KeyBindingsTree {
    ...
    protected addMappingHelper(
        ...
        if (child instanceof Motion) {
            throw new Error(`Multiple registrations for key sequence ${keys.slice(0, index + 1)}: ${name} and ${child.name}`);
        }
        ...
        if (index === keys.length - 1) {
            if (child != null) {
                throw new errors.GenericError(`Multiple registrations for key sequence ${keys.slice(0, index)}: ${name} and ${child.lastAdded && child.lastAdded[0]}`);
            }
